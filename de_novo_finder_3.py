...
deletions (indels). To skip indels, use the -i flag.
...
def process_line(line, args):
    ...
    if args.acceptTST: # accept TruthSensitivity Tranche variants
        if not line[filter_pos].startswith(('PASS',
    ...
    # Remove indels if the -i flag was used
    if args.noindels:
        if line[ref_allele] not in ('A', 'C', 'G', 'T'):
            passedcheck = False
        if line[alt_allele] not in ('A', 'C', 'G', 'T'):
            passedcheck = False
...
def VEP_annotate(var_annotation):
    "Take variant annotation and extract gene name and mutation type"
    # Set for VEP annotation
    # Taken from Jack Kosmicki
    var_annotation = var_annotation.split(';')
    ...
    for entry in var_annotation:
        if re.search('SEVERE_GENE', entry):
            gene_name = entry.split('=')[1]
        elif re.search('SEVERE_IMPACT', entry):
            functional_class = entry.split('=')[1]
        else:
            continue
...
def process_autosome_variant(line, Fam, PL_pos, AD_pos, DP_pos, args,
    ...
    for column, entry in enumerate(line):
        ...
        if args.annotatevar:
            ...
            (var_gene, var_category) = VEP_annotate(variant_annotation)
            res_indiv = [
                line[0], line[1], line[2], line[3], line[4],
                labels[column], labels[dad_pos], labels[mom_pos],
                child_sex, child_aff_status, child_PL[0], dad_PL[1],
...
def main(vcffile, Fam, args):
	...
	chrom_under_study = line[0]
        ...
        # Update the ESP count dictionary if this line has a new chromosome
        ...
        if ',' in alt_allele:
            ...
            # List comprehension to determine that both alternative alleles are single bases
            if all(x in ('A', 'C', 'G', 'T') for x in alt_alleles):
                multi_flag = True
            else:
                continue
            ...
            if ((int(line[1]) < 2699520) or
                ...
                if multi_flag == True:
                    process_multi_variant(line, Fam, PL_pos, AD_pos, DP_pos, args,
                                          ...
                                          chrom_under_study, am_kid, who_dad, who_mom)
    ...
    sys.stderr.write('Number of lines with no PL information: {0}\n'.format(
            no_PL_lines))
    sys.stderr.write('Numver of lines with no AD information: {0}\n'.format(
            no_AD_lines))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--noindels', action='store_true',
                        dest='noindels', help='Skip indels')

    parser.add_argument('-q', '--acceptTSTqual', action='store_true',
                        dest='acceptTST', help='''
                        Accept the TruthSensitivityTranche quality filter''')
    # NOTE: -q flag has been flipped from previous versions.
    # Using -q now means that you want to look at the TST variants
    ...
    # Note: VEP annotation code comes from Jack Kosmicki
